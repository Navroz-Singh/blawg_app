...
export default function BlogClientContent({ blog: initialBlog, blogid, htmlContent, headings }) {
    const auth = useAuth() || {};
    const { user } = auth;
    ...
    // Apply custom styling to headings when the component mounts
    useEffect(() => {
        try {
            // Add IDs to headings if they don't have them already
            ...
            headingElements.forEach((heading) => {
                ...
                heading.addEventListener('mouseenter', () => {
                    ...
                    linkIcon.innerHTML = ' 🔗';
                    linkIcon.className = 'heading-link-icon text-blue-500 opacity-50 hover:opacity-100';
                ...
                heading.addEventListener('click', (e) => {
                    // Don't trigger if clicking on a link inside the heading
    ...
    return (
        <>
            ...
            <div className="lg:hidden sticky top-0 z-20 bg-white dark:bg-gray-800 p-4 border-b border-gray-200 dark:border-gray-700 shadow-sm">
                <button 
                    onClick={() => setShowMobileSidebar(!showMobileSidebar)}
                    className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors duration-200 w-full"
                >
                    <FaBars className="w-4 h-4" />
                    <span>{showMobileSidebar ? 'Hide Table of Contents' : 'Show Table of Contents'}</span>
                </button>
                
                {/* Mobile sidebar */}
                {showMobileSidebar && (
                    <div className="absolute top-full left-0 w-full bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4 shadow-lg z-10 max-h-[60vh] overflow-y-auto custom-scrollbar">
                        <div className="py-2">
            ...
            <div className="flex w-full">
                {/* Desktop sidebar */}
                <BlogSectionSidebar headings={headings} />
